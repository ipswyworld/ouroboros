...
#![cfg(disabled)]
use ouro_dag::bft::consensus::{HotStuff, Vote, Proposal, BFTNode};
...
async fn setup_test_nodes(pool: PgPool, count: usize) -> (Vec<TestNode>, BroadcastHandle) {
    ...
    for i in 0..count {
        ...
        let kp = crypto_bridge::generate_keypair_write(std::path::Path::new(&format!("{}.key", node_id))).unwrap();
    ...
    for i in 0..count {
        let node_id = format!("node{}", i);
        let peers: Vec<String> = node_ids.iter().filter(|&id| *id != node_id).cloned().collect();
        let kp = crypto_bridge::load_keypair(std::path::Path::new(&format!("{}.key", node_id))).unwrap();
        let hs = HotStuff::new(node_id, peers, broadcaster.clone(), bft_state.clone(), validators.clone(), 2000);
...
async fn test_propose_vote_qc_flow() {
    ...
    for node in &mut nodes {
        broadcaster.add_peer_tx(node.hs.id.clone(), tx.clone()).await;
    }
