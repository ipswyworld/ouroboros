...
impl NodeIdentity {
    ...
    fn get_machine_identifier() -> String {
        ...
        // Fallback to random UUID (won't be consistent across restarts)
    ...
    /// Update total uptime
    pub fn update_uptime(&mut self, additional_secs: u64, path: &Path) -> Result<(), Box<dyn Error>> {
...
mod tests {
    ...
    fn test_set_public_name() {
        ...
        identity.set_public_name("Alice's Node".to_string(), &path).unwrap();
