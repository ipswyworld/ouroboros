...
# Usage: DATABASE_URL=postgres://user:pass@host:port/db ./ops/wait-for-db-and-start.sh
...
echo "Applying migrations in /app/migrations (idempotent SQL expected)"
...
mkdir -p /usr/src/ouro_dag && ln -sfn /app/migrations /usr/src/ouro_dag/migrations

echo "Starting Ouroboros node"
exec /app/ouro_dag start