...
### 1. **Node Number / Persistent Identity** ⭐⭐⭐
...
**API Endpoints**:
- `GET /identity` → Returns node number, ID, stats
- `POST /identity/name` → Set custom name
...
4. Show notification: "Update available: v0.3.1 → v0.4.0"
...
**API Endpoints**:
- `GET /updates/check` → {available: true, version: "0.4.0"}
- `POST /updates/install` → Triggers update
- `GET /updates/status` → {updating: false, last_check: "..."}

**Time Estimate**: 12 hours

---

### 3. **Connect Node to Wallet** ⭐⭐⭐
...
✅ Node #42 linked to wallet ouro1abc...xyz
...
5. **Rewards Tracking**
   ...
   - Claim button: Moves from pending → available
...
├── Cargo.toml
├── src/
│   ├── crypto.rs      # Key generation, signing
│   ├── rpc.rs         # Connect to full node
│   ├── wallet.rs      # Wallet logic
│   └── main.rs        # CLI entry point
└── ui/                # Tauri frontend (if desktop)
```
...
### **Phase 3: Ecosystem** (Week 4-6)
🔄 **Choose One** (or split across versions):
- Option A: **Wallet MVP** (30h) → v0.3.0
- Option B: **Microchain SDK** (40h) → v0.4.0

**Recommendation**:
- **v0.3.0**: Node features + Wallet MVP
- **v0.4.0**: Microchain SDK (needs more mature mainchain)
...
**v0.3.0 Target Date**: January 15, 2026 (4-6 weeks)

**Included**:
- ✅ Node Identity & Number
- ✅ Wallet Linking
- ✅ Automatic Updates
- ✅ Wallet MVP (CLI + Desktop)
- ✅ Peer connection fix (already done)

**Total Estimated Effort**: ~58 hours (~1.5 weeks full-time)

**v0.4.0 Preview** (February 2026):
- Microchain SDK
